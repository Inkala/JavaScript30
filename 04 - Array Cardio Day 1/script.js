...
const fifteen = inventors.filter(inv => (
  inv.year >= 1500 && inv.year < 1600
));
...
const bDates = inventors.sort((a, b) => b.year - a.year);

// const bDates = inventors.sort((a, b) => a.year > b.year ? 1 : -1); // For not numbers

// console.table(bDates);
...
const de = boulevards.map(bul => bul.textContent);
const bulList = de.filter(x => x.includes('de'));
...
const ele = data.reduce((obj, item) => {
...
// const ele = {}
// data.map(item => ele[item] ? ele[item]++ : ele[item] = 1)
